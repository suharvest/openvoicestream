...
[[ "$(python3 -c 'import json,sys; print(str(json.load(open(sys.argv[1]))["published_to_hf"]).lower())' "$ARTIFACT/manifest.json")" == false ]]
if docker container inspect "$NAME" >/dev/null 2>&1; then
  echo "refusing to reuse existing canary: $NAME" >&2
  exit 1
fi
...
docker run -d \
  ...
  --network "$NETWORK" \
  --env-file "$ENV_FILE" \
  -e OVS_PROFILE=jetson-edgellm-v091-qwen3ttsbase \
...
docker inspect "$NAME" > "$EVIDENCE/canary.inspect.full.json"
chmod 0600 "$EVIDENCE/canary.inspect.full.json"
...
if [[ "$ready" != true ]]; then
  echo "canary failed to become ready" >&2
  exit 1
fi
date --iso-8601=seconds > "$EVIDENCE/ready-at.txt"
echo "BASE_N1_CANARY_READY"
