...
capture() {
  ...
  curl -fsS http://127.0.0.1:18623/readyz > "$EVIDENCE/readyz-after.json" || post_rc=1
  ...
  [[ "$rc" != 0 ]] || echo "CUSTOMVOICE_N1_FUNCTIONAL_PASS_CLONE_GATE_BLOCKED"
  exit "$rc"
}
trap capture EXIT
...
curl -fsS http://127.0.0.1:18623/readyz > "$EVIDENCE/readyz-before.json"
