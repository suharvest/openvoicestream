...
IMAGE=seeed-local-voice:v0.9.1-edgellm-runtime-r3-cvfix-ef27c98-20260726
IMAGE_ID=sha256:04f3e582da5975f636105e16ff8824a664454692dd921fad958fcea1e0de2bee
...
[[ "$(docker image inspect -f '{{index .Config.Labels "com.seeed.voxedge.revision"}}' "$IMAGE")" == \
  8e043d3e676362e103d710a67b53605efc01a241 ]]
[[ -r "$ENV_FILE" ]]
...
if docker container inspect "$NAME" >/dev/null 2>&1; then
  echo "refusing to reuse existing r3 CustomVoice N1 canary: $NAME" >&2
  exit 1
fi
...
[[ "$ready" == true ]] || { echo "r3 CustomVoice N1 failed to become ready" >&2; exit 1; }

docker exec "$NAME" env > "$EVIDENCE/resolved-env.full.txt"
grep -E '^(OVS_PROFILE|OVS_TTS_MODEL_ID|EDGE_LLM_ASR_MAX_CONCURRENT|EDGE_LLM_ASR_ENGINE_DIR|EDGE_LLM_TTS_TALKER_DIR|EDGE_LLM_TTS_CP_DIR|OVS_TTS_WORKER_CONCURRENCY|OVS_MAX_CONCURRENT_SESSIONS)=' \
...
grep -Fx 'qwen3-tts-customvoice' "$EVIDENCE/resolved-env.txt"
...
echo "R3_CUSTOMVOICE_N1_CANARY_READY"
