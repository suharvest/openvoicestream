...
把 decoder 换成 CPU 上带 KV cache 的 ONNX 图之后，**每块板都是又快又准**：RK3588 英文长句 WER 10.44% → **6.33%**，RTF 0.149 → **0.061**。

**窗口长度是对话/转录之间的取舍杠杆，而且是可配置的。** 同一块 RK3588、同一个模型、同一套代码，只把 encoder 窗口从 20 秒换成 10 秒：TTFT 297 ms → **123 ms**（2.4×），短句 WER 13.37% → **11.37%**；代价是长音频必须切块，长句 WER 6.33% → 14.26%。
...
| **RK3588** | base / 20s / **hybrid** | 13.37% | **6.33%** | 44.04 | 28.33 (20.47) | 297 ms | 0.061 |
