...
def test_default_spec(engine):
    ...
    # Identity is a content digest, not name+size: two different models of the
    # same size must not look like the same artifact.
    assert spec["onnx_sha256"] == hashlib.sha256(b"x" * 1024).hexdigest()


def test_missing_onnx_does_not_raise(tmp_path):
    spec = md._sensevoice_build_spec(str(tmp_path / "nope.onnx"))
    assert spec["onnx_sha256"] == ""
...
def test_redownloaded_onnx_marks_engine_stale(engine):
    ...
    assert stale is not None and "onnx_sha256" in stale


def test_same_size_different_contents_marks_engine_stale(engine):
    """Name+size could not tell these apart; the digest must."""
    onnx, plan = engine
    _write_sidecar(plan, md._sensevoice_build_spec(str(onnx)), trt=_trt_version())

    onnx.write_bytes(b"z" * 1024)  # SAME size, different model
    stale = md._sensevoice_engine_staleness(
        str(plan), md._sensevoice_build_spec(str(onnx))
    )
    assert stale is not None and "onnx_sha256" in stale
