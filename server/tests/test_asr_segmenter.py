...
def test_fixed_bounds_absorb_tiny_tail():
    """A sliver tail folds into the previous slice WITHOUT exceeding the limit.

    Going over max_segment_seconds would silently truncate when that limit is a
    backend's hard fixed-input ceiling -- the exact tail the fold exists to keep.
    """
    limit, overlap = 10.5, 0.4
    bounds = seg.fixed_segment_bounds(20.8, limit, overlap)

    assert bounds[-1][1] == pytest.approx(20.8), "tail must still be covered"
    for start, end in bounds:
        assert end - start <= limit + 1e-6, f"slice {start}-{end} exceeds {limit}"
    # No gap: consecutive slices still touch or overlap.
...
# ── 重叠去重（codex 审核 P1）─────────────────────────────────────────
# 相邻切片故意重叠 DEFAULT_OVERLAP_SECONDS，好让跨切点的词至少被完整看到一次。
# 代价是重叠区的音频被解码两次，同一批词会同时出现在两段结果里；直接拼接就会
# 复读。join 时要把后一段开头与前一段结尾重复的部分去掉。

@pytest.mark.parametrize("prev,cur,expected", [
    # 英文：整词重复
    ("the tribal chieftain called for the boy",
     "for the boy and presented him with gold",
     "and presented him with gold"),
    # 中文：无空格，按字符去重
    ("重点呢想谈三个问题首先呢", "首先呢就是这一轮全球金融动荡", "就是这一轮全球金融动荡"),
    # 无重复时原样保留
    ("hello world", "goodbye now", "goodbye now"),
    # 只是共享词的一部分，不能吞掉（boy / boyish）
    ("called for the boy", "boyish grin", "boyish grin"),
    # 空输入
    ("", "anything", "anything"),
    ("something", "", ""),
])
def test_strip_overlap_repeat(prev, cur, expected):
    assert seg.strip_overlap_repeat(prev, cur) == expected


def test_join_drops_duplicated_overlap():
    parts = [
        "the tribal chieftain called for the boy",
        "for the boy and presented him with gold",
    ]
    joined = seg.join_segment_texts(parts)
    assert joined == "the tribal chieftain called for the boy and presented him with gold"
    assert joined.count("for the boy") == 1, "重叠区的词不能出现两次"


def test_join_cjk_drops_duplicated_overlap():
    joined = seg.join_segment_texts(["重点呢想谈三个问题首先呢", "首先呢就是这一轮全球金融动荡"])
    assert joined == "重点呢想谈三个问题首先呢就是这一轮全球金融动荡"
    assert joined.count("首先呢") == 1


def test_join_without_overlap_is_unchanged():
    """无重叠时行为与改动前完全一致。"""
    assert seg.join_segment_texts(["seg0", "seg1", "seg2"]) == "seg0 seg1 seg2"
    assert seg.join_segment_texts(["中文一", "中文二"]) == "中文一中文二"


# ── 分段挂在共用解码点，因此对所有传输一致 ──────────────────────────
# 上游把非流式 ASR 收敛到 server/core/api_execution.execute_asr()，原生 /asr、
# /v1/asr 与 OpenAI 适配器都经由它，所以分段只需接在那一处。这里用原生 /asr
# 覆盖语义；OpenAI 那条路径由上游自己的 test_openai_compat.py 覆盖契约。

def test_segmentation_lives_at_the_shared_decode_point():
    """分段必须挂在 execute_asr，而不是某一个传输的路由里。"""
    import inspect
    from server.core import api_execution

    src = inspect.getsource(api_execution)
    assert "maybe_transcribe_segmented" in src, "共用解码点应调用分段器"
    assert "run_in_executor" in src, "阻塞解码应移出 event loop"


def test_long_audio_segmented_through_native_asr(harness):
    """端到端：长音频经原生 /asr 被分段，且元数据齐全。"""
    be = _CountingASRBackend()
    client = harness(be)
    r = _post_asr(client, _make_wav(30.0))
    assert r.status_code == 200, r.text
