...
def build_sensevoice_trt_config(
    ...
    # -- max_concurrent: env → profile asr_max_slots → 1. Mirrors the
    #    paraformer chain above, but defaults to 1 rather than 2: each extra
    #    TRT execution context holds its own resident I/O buffers (~33.6 MiB
    #    for the (1,344,25055) fp32 output alone) plus activation scratch, so
    #    raising it is an explicit, per-device decision.
    ...
    # ``max_concurrent`` exists only in voxedge builds that carry the
    # SenseVoice context pool. Passing it unconditionally would TypeError on an
    # older voxedge, so a deploy could not roll the two repos independently —
    # feed it only when the dataclass declares it.
    if any(f.name == "max_concurrent" for f in dataclasses.fields(SenseVoiceTRTConfig)):
        ...
        logger.warning(
            ...
            "voxedge build has no SenseVoice context pool — staying at 1 slot",
            max_concurrent,
        )
...
def concurrency_capability_for_spec(spec, cls, kind, profile=None):
    ...
    # voxedge backends are not consistent about the attribute name they read
    # their config from — paraformer/trt_edge_llm use ``_config``, sensevoice
    # uses ``_cfg``. Setting only one silently produced a stub whose
    # ``concurrency_capability()`` raised AttributeError, which the caller then
    # swallowed into ``ConcurrencyCapability.default()`` (max_concurrent=1) —
    # i.e. every knob for that backend looked wired up and did nothing. Set
    # both rather than depend on which name a given backend happens to use.
    stub._config = config
    stub._cfg = config
    try:
